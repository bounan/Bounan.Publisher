import type { SceneRecognisedNotificationItem } from '../../../../third-party/common/ts/interfaces';
...
export const createTextForTopicName = (animeInfo: ShikiAnimeInfo, dub: string): string => {
  return [animeInfo.russian || animeInfo.name, dub, animeInfo.airedOn?.year].filter(Boolean).join(' | ');
};
...
export const createTextForHeaderPost = (animeInfo: ShikiAnimeInfo, dub: string): string => {
  ...
  animeInfo.synonyms?.forEach((name) => {
...
export const createTextForEpisodePost = (
  ...
  const has_episodes =
    (animeInfo.episodes && animeInfo.episodes > 1) || (animeInfo.episodesAired && animeInfo.episodesAired > 1);
